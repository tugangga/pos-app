<x-admin.layout>
    <div class="flex justify-between  border-b border-gray-400/20 pb-3 mb-4">
        <div class="flex justify-between grow space-x-3 relative ">
            <h2 class="h2 text-2xl font-semibold text-gray-900 ">{{ $title }}</h2>
            @if (session('sukses'))
        ...
        <a href="{{ route('user.create') }}" class="btn-primary">Tambah User Baru</a>
    </div>

    
    <div class="relative overflow-x-auto bg-gray-200 shadow-xs rounded-lg border border-gray-200/20">
        <table class="table-auto md:table-fixed w-full text-md  text-left">
            <thead>
                <tr>
                    <th class="px-6 py-3 font-medium w-10">No</th>
                    <th class="px-6 py-3 font-medium">Username</th>
                    <th class="px-6 py-3 font-medium">Nama</th>
                    <th class="px-6 py-3 font-medium sm:w-24">Hak Akses</th>
                    <th class="px-6 py-3 font-medium sm:w-24">status</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-400/20 bg-white">
                    ...
                    <td class="px-6 py-3 font-light" >{{ $user->username }}</td>
                    <td class="px-6 py-3 font-light" >{{ $user->nama }}</td>
                    <td class="px-6 py-3 font-light" >{{ $user->akses }}</td>
                    <td class="px-6 py-3 font-light" >{{ $user->status }}</td>
                    <td class="px-6 py-3 flex space-x-2">
                        
                        <a href="/user/{{ $user->id }}/edit"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5"  class="size-6 inline text-green-800 stroke-[]1.5]">
